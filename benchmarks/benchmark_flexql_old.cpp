...
static const long long DEFAULT_INSERT_ROWS = 10000LL;
...
static bool run_insert_benchmark(FlexQL *db, long long target_rows) {
    ...
    while (inserted < target_rows) {
        ...
        int current_batch = 0;
        while (current_batch < INSERT_BATCH_SIZE && inserted < target_rows) {
            long long id = inserted + 1;
            if (current_batch > 0) ss << ",";
            ss << "(" << id
               << ", 'user" << id << "'"
               << ", 'user" << id << "@mail.com'"
               << ", " << (1000.0 + (id % 10000))
        ...
        if (flexql_exec(db, ss.str().c_str(), nullptr, nullptr, &errMsg) != FLEXQL_OK) {
            cout << "[FAIL] INSERT BIG_USERS -> " << (errMsg ? errMsg : "unknown error") << "\n";
