...
static const int INSERT_BATCH_SIZE = 5000; // if you implement batch inserts in flexql, you can increase this for better performance
...
static bool run_insert_benchmark(FlexQL *db, long long target_rows) {
    ...
    auto last_time = bench_start;
    long long last_inserted = 0;
    ...
    while (inserted < target_rows) {
        ...
        if (inserted >= next_progress || inserted == target_rows) {
            ...
            long long batch_elapsed = duration_cast<milliseconds>(current_time - last_time).count();
            long long batch_rows = inserted - last_inserted;
            long long batch_throughput = (batch_elapsed > 0) ? (batch_rows * 1000LL / batch_elapsed) : batch_rows;
            
            cout << "Progress: " << inserted << "/" << target_rows 
                 << " | Last " << batch_rows << " rows: " << batch_elapsed << " ms"
                 << " | Throughput: " << batch_throughput << " rows/sec\n";
